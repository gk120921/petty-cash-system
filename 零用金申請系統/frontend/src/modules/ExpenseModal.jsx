        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col lg:flex-row">
          ...
          <form onSubmit={onAddExpense} className="w-full lg:w-1/2 p-10 space-y-8 bg-white">
            <div className="grid grid-cols-2 gap-6">
              ...
              <FormGroup label="1.2 經辦人 Handler">
                <select className="w-full bg-slate-50 border-none rounded-2xl px-6 py-4 outline-none font-bold focus:ring-2 focus:ring-brand-500/20" value={formData.handler_personnel_id || ''} onChange={e => setFormData({ ...formData, handler_personnel_id: e.target.value })}>
                  <option value="">選擇經辦人 Select...</option>
                  {personnel.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                </select>
              </FormGroup>
            </div>
