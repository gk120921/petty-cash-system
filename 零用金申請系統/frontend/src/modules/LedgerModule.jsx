...
export function LedgerModule({ permissions }) {
  const {
    ...
    API_BASE, language, toggleLanguage, user, hasPermission
  } = useExpenseContext();
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8 pb-20">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="bg-rose-50 rounded-xl px-5 py-2 font-black text-xl text-rose-600 min-w-[140px] text-center border border-rose-100/50">
                  ${Number(balanceThreshold).toLocaleString()}
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="bg-white rounded-[32px] p-8 border border-slate-200 shadow-xl shadow-slate-200/40 flex flex-col md:flex-row items-center gap-6 justify-center lg:justify-start">
        <div className="flex flex-wrap items-center gap-4 w-full">
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                link.href = url;
                const filename = `PettyCash_Report_New_${new Date().toISOString().split('T')[0]}.xlsx`;
          ...
          <button
            disabled={exportingLegacy || loading}
            onClick={async () => {
              ...
              try {
                ...
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', `PettyCash_Report_Legacy_${new Date().toISOString().split('T')[0]}.xlsx`);
                document.body.appendChild(link);
                link.click();
                setTimeout(() => {
                  document.body.removeChild(link);
                ...
                alert('?臬憭望?: ' + err.message);
              } finally {
                setExportingLegacy(false);
              }
            }}
            className="flex items-center gap-3 px-8 py-4 bg-slate-700 text-white rounded-2xl font-bold hover:bg-slate-800 transition-all shadow-lg disabled:opacity-50 min-w-[240px] flex-1 md:flex-none justify-center"
          >
            {exportingLegacy ? (
              <span className="flex items-center gap-2">
                <Loader2 className="animate-spin h-5 w-5 text-white" />
                {language === 'zh' ? '產生原報表中...' : 'Generating legacy report...'}
              </span>
            ) : (
              <>
                <FileText className="w-5 h-5" />
                {language === 'zh' ? '下載原報表' : 'Download Legacy Report'}
              </>
            )}
          </button>
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                const link = document.createElement('a');
                link.href = url;
                link.setAttribute('download', `PettyCash_Inventory_${new Date().toISOString().split('T')[0]}.xlsx`);
                document.body.appendChild(link);
                link.click();
      ...
      <section className="bg-white rounded-[32px] p-8 border border-slate-200 shadow-xl shadow-slate-200/40">
        ...
        <div className="flex flex-wrap gap-2 mb-8 bg-slate-50/50 p-2 rounded-2xl border border-slate-100">
          ...
          {(() => {
            const months = [];
            expenses.forEach(e => {
              const sourceDate = e.reimbursement_date || e.invoice_date;
            ...
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden mb-8">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pt-6 border-t border-slate-100">
                ...
                <div className="space-y-2 col-span-1 md:col-span-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase ml-1">報帳日期區間 Reimbursement Period</label>
